...
/*   Updated: 2022/02/04 14:09:29 by sangkkim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf_bonus.h"

int	set_precision(t_format *format, const char *f_string);
int	set_width(t_format *format, const char *f_string);
...
t_format	get_format(const char *f_string)
{
	...
	while (*f_string)
	{
		if (*f_string == '-')
			...
			f_string += set_precision(&format, f_string);
		else if (isdigit(*f_string))
			f_string += set_width(&format, f_string);
		else if (*f_string == '#')
